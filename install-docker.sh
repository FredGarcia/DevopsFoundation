#!/usr/bin/env bash
# =====================================================================
#  DOF-PREP — Installeur Docker
#  Genere le contexte de build (Dockerfile + application + compose).
#  Image de base : python:3.12-slim. ZERO dependance applicative.
# =====================================================================
set -euo pipefail

APP_DIR="devops-foundation-prep-docker"
echo ">> Preparation du contexte Docker dans ./$APP_DIR"
mkdir -p "$APP_DIR"

# --- Application (source identique sur les 3 plateformes) ---
cat > "$APP_DIR/devops_foundation_prep.py" <<'DOFPREP_APP_PY_EOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
# --- Modele de configuration (.env.example) ---
cat > "$APP_DIR/.env.example" <<'DOFPREP_ENV_EXAMPLE_EOF'
# .env.example — DOF-PREP : variables d'environnement (toutes OPTIONNELLES)
# Copier ce fichier en .env et decommenter/adapter les lignes voulues.
# Le lanceur start.sh charge automatiquement un fichier .env present a cote.

# --- Reseau / stockage ---
# DOF_HOST=127.0.0.1             # 0.0.0.0 en conteneur
# DOF_PORT=8765
# DOF_DB_PATH=dofprep.sqlite3
# DOF_OPEN_BROWSER=1             # 0 pour ne pas ouvrir le navigateur (serveur / Docker)
# DOF_LOCALE=fr

# --- Couche de second ordre (cybernetique des systemes observants) ---
# Calibration metacognitive + auto-calibration de l'instrument de mesure.
# Desactivee par defaut ; mettre a 1 pour l'activer par defaut.
# DOF_SECOND_ORDER=0
#
# Parametres de la BOUCLE REFLEXIVE (bornes de surete appliquees au demarrage) :
#
#   DOF_ELO_K — gain de la mise a jour mutuelle aptitude <-> difficulte latente
#   (Elo leger sur echelle logit). Plus eleve = adaptation plus rapide mais plus
#   nerveuse. Borne a [0,1]. Defaut 0.08.
# DOF_ELO_K=0.08
#
#   DOF_CALIB_MIN_RECORDS — nombre minimal de predictions de confiance avant que
#   la calibration n'influe sur la preparation (anti-oscillation). Entier >= 1.
#   Defaut 20.
# DOF_CALIB_MIN_RECORDS=20
#
#   DOF_CALIB_FLOOR — plancher du facteur de calibration : la preparation peut
#   etre amortie par une mauvaise calibration, jamais effondree. Le seuil officiel
#   de reussite (65 %) n'est JAMAIS abaisse. Borne a [0,1]. Defaut 0.60.
# DOF_CALIB_FLOOR=0.60
DOFPREP_ENV_EXAMPLE_EOF

# --- Dockerfile ---
cat > "$APP_DIR/Dockerfile" <<'DOFPREP_DOCKERFILE_EOF'
FROM python:3.12-slim
WORKDIR /app
COPY devops_foundation_prep.py /app/
RUN mkdir -p /app/data
ENV DOF_HOST=0.0.0.0 \
    DOF_PORT=8765 \
    DOF_OPEN_BROWSER=0 \
    DOF_DB_PATH=/app/data/dofprep.sqlite3
EXPOSE 8765
CMD ["python", "devops_foundation_prep.py"]
DOFPREP_DOCKERFILE_EOF

# --- docker-compose.yml (persistance SQLite via volume nomme) ---
cat > "$APP_DIR/docker-compose.yml" <<'DOFPREP_COMPOSE_EOF'
services:
  dof-prep:
    build: .
    container_name: dof-prep
    ports:
      - "8765:8765"
    volumes:
      - dofprep-data:/app/data
    restart: unless-stopped
    # Couche de second ordre (optionnelle) : decommentez pour l'activer/regler.
    # environment:
    #   - DOF_SECOND_ORDER=1
    #   - DOF_ELO_K=0.08
    #   - DOF_CALIB_MIN_RECORDS=20
...
echo ""
echo ">> Contexte Docker pret dans ./$APP_DIR"
echo "   Construire et lancer :"
echo "      cd $APP_DIR && docker compose up --build -d"
echo "   Puis ouvrir : http://localhost:8765/"
echo "   (Variante sans compose : docker build -t dof-prep . && docker run -d -p 8765:8765 -v dofprep-data:/app/data dof-prep)"
