# =====================================================================
#  DOF-PREP - Installeur Windows (PowerShell)
#  Plateforme de preparation a la certification DevOps Foundation.
#  ZERO dependance : Python 3 standard uniquement. Fonctionne hors-ligne.
#  Lancement :  powershell -ExecutionPolicy Bypass -File install.ps1
# =====================================================================
$ErrorActionPreference = 'Stop'

$AppDir = 'devops-foundation-prep'
Write-Host ">> Installation dans .\$AppDir"
New-Item -ItemType Directory -Force -Path $AppDir | Out-Null
$dest = (Resolve-Path $AppDir).Path
...
if __name__ == "__main__":
    main()
'@
[System.IO.File]::WriteAllText((Join-Path $dest 'devops_foundation_prep.py'), $app, $utf8NoBom)

# --- Lanceur ---
$bat = @'
@echo off
cd /d "%~dp0"
python devops_foundation_prep.py %*
'@
$bat = ($bat -replace "`n", "`r`n")
[System.IO.File]::WriteAllText((Join-Path $dest 'start.bat'), $bat, $utf8NoBom)
...
'@
[System.IO.File]::WriteAllText((Join-Path $dest '.env.example'), $envx, $utf8NoBom)

# --- Verification de Python ---
$py = Get-Command python -ErrorAction SilentlyContinue
if (-not $py) {
  Write-Host "!! Python 3 est requis mais introuvable. Installez-le (python.org) puis lancez start.bat"
} else {
  Write-Host ""
  Write-Host ">> Installation terminee."
  Write-Host "   Lancer :  cd $AppDir ; .\start.bat"
  Write-Host "   Puis ouvrir : http://localhost:8765/"
  Write-Host "   Configuration optionnelle : copiez .env.example puis definissez les variables voulues."
}
