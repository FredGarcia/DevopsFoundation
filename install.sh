#!/usr/bin/env bash
# =====================================================================
#  DOF-PREP — Installeur Linux / macOS
...
APP_DIR="devops-foundation-prep"
...
# --- Lanceur ---
cat > "$APP_DIR/start.sh" <<'DOFPREP_START_SH_EOF'
#!/usr/bin/env bash
cd "$(dirname "$0")"
# Charge un fichier .env present a cote (variables exportees automatiquement).
[ -f .env ] && set -a && . ./.env && set +a
exec python3 devops_foundation_prep.py "$@"
DOFPREP_START_SH_EOF
...
if ! command -v python3 >/dev/null 2>&1; then
  echo "!! Python 3 est requis mais introuvable. Installez-le puis relancez ./$APP_DIR/start.sh"
  exit 1
fi

echo ""
echo ">> Installation terminee."
echo "   Lancer :  cd $APP_DIR && ./start.sh"
echo "   Puis ouvrir : http://localhost:8765/"
echo "   Configuration optionnelle : cp .env.example .env  puis editez .env"
